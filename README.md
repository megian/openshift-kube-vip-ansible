...
Kube-vip provides an easy to manage VIP/LoadBalancer for the control plane based on the [https://en.wikipedia.org/wiki/Raft_(computer_science)](Raft consensus algorithm). It is configured as Kubernetes static pod, so there is no recursive dependency to the control plane needs to be up.
